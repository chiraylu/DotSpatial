...
namespace DotSpatial.Symbology
{
    ...
    public static class DesktopRasterExt
    {
        ...
        public static void DrawToBitmap(this IRaster raster, IRasterSymbolizer rasterSymbolizer, Bitmap bitmap, IProgressHandler progressHandler = null)
        {
            ...
            if (rasterSymbolizer == null) throw new ArgumentNullException(nameof(rasterSymbolizer));
        ...
        public static List<double> GetRandomValues(this IRaster raster, int sampleSize)
        {
            ...
            if (numRows * (long)numCols < (long)sampleSize * 5 && numRows * (long)numCols < int.MaxValue)
            {
                ...
                if (count < int.MaxValue) count = (int)longcount;
        ...
        public static ISet<double> GetUniqueValues(this IRaster raster, int maxCount, out bool overMaxCount)
        {
            ...
            if (raster.DataType == typeof(byte) || raster.DataType == typeof(int) || raster.DataType == typeof(sbyte) || raster.DataType == typeof(uint) || raster.DataType == typeof(short) || raster.DataType == typeof(ushort))
            {
            ...
            for (var row = 0; row < numRows; row++)
            {
                for (var col = 0; col < numCols; col++)
                {
                    ...
                    if (result.Add(val))
                    {
                        ...
                        if (result.Count == totalPossibleCount) return result;
                    }
                }
            }
                ...
                if (set.Contains(value))
                {
                    ...
                    switch (set.GradientModel)
                    {
                        ...
                        case GradientModel.Exponential:
                            ht = dVal;
                            if (ht < 1) ht = 1.0;
                            if (range > 1) p = Math.Pow(ht - lowVal, 2) / Math.Pow(range, 2);
                            else return set.Color;

                            break;
                        case GradientModel.Logarithmic:
                            ht = dVal;
                            if (ht < 1) ht = 1.0;
                            if (range > 1.0 && ht - lowVal > 1.0) p = Math.Log(ht - lowVal) / Math.Log(range);
                            else return set.Color;
            ...
            where T : struct, IComparable<T>
        {
            ...
            foreach (var c in categories)
            {
                ...
                {
                    if (c.Range.Maximum < testMin) cs.Max = cs.Min;
                    else cs.Max = (T)Convert.ChangeType(c.Range.Maximum.Value, typeof(T));
                }

                if (c.Range.Minimum != null && c.Range.Minimum > testMin)
                {
                    if (c.Range.Minimum > testMax) cs.Min = Global.MaximumValue<T>();
                    else cs.Min = (T)Convert.ChangeType(c.Range.Minimum.Value, typeof(T));
                }
        ...
        private static T GetNoData<T>(Raster<T> raster)
            where T : IEquatable<T>, IComparable<T>
        {
            // Get nodata value.
            ...
            where T : struct, IComparable<T>
        {
            ...
            public Argb Color { get; set; } // for non bivalue case.
            ...
            public bool Contains(T value)
            {
                ...
                if (Min == null) return MaxInclusive ? value.CompareTo(Max.Value) <= 0 : value.CompareTo(Max.Value) < 0;
                if (Max == null) return MinInclusive ? value.CompareTo(Min.Value) >= 0 : value.CompareTo(Min.Value) > 0;
